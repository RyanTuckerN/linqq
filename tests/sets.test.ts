import linq, { EqualityComparer } from "../src";
...
test("union(), - equality comparer", () => {
  ...
  const compare = (c: EqualityComparer<{ id: number; name: string }>) =>
    linq([
      { id: 1, name: "Alice" },
      { id: 2, name: "Bob" },
    ])
  ...
  expect(compare(LinqUtils.defaultEqualityComparer)).toBe(4);
  expect(compare((a, b) => a.id === b.id)).toBe(3);
});
...
test("intersect(), - equality comparer", () => {
  ...
  expect(compare(LinqUtils.defaultEqualityComparer)).toBe(0);
  expect(compare((a, b) => a.id === b.id)).toBe(1);
});
...
test("except(), - equality comparer", () => {
  expect(
    linq([obj1, obj2])
      .except([obj3, obj4], (a, b) => a.address === b.address)
      .toArray().length,
  ).toBe(0);
  ...
  expect(compare(LinqUtils.defaultEqualityComparer)).toBe(2);
  expect(compare((a, b) => a.id === b.id)).toBe(1);
});
